...
| 👥 **Assign groups** | Replace, add to, or reset the include/exclude persona groups of selected policies. Missing groups are created via Graph and are **always role-assignable** (`isAssignableToRole: true`) — the flag is immutable, so it has to be right at creation. Entra forbids role-assignable + dynamic membership, so a dynamic template is created assigned and its membership rule is reported for manual setup rather than silently producing a group that can never hold a role. | **yes** |
